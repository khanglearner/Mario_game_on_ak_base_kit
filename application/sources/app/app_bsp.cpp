...
void btn_up_callback(void* b) {
	...
	case BUTTON_SW_STATE_LONG_PRESSED: {
		APP_DBG("[btn_up_callback] BUTTON_SW_STATE_LONG_PRESSED\n");
			if(mar_game_state == GAME_PLAY) {
				timer_set(AC_TASK_DISPLAY_ID,MAR_GAME_MAP_MOVE_LEFT_TIMER_SIG, 100, TIMER_PERIODIC);
			}
		else{
	...
	case BUTTON_SW_STATE_RELEASED: {
		...
		else if(mar_game_state == GAME_PLAY) {
			timer_remove_attr(AC_TASK_DISPLAY_ID, MAR_GAME_MAP_MOVE_LEFT_TIMER_SIG);
		}
		else {
...
void btn_down_callback(void* b) {
	...
	case BUTTON_SW_STATE_PRESSED: {
		...
		if(mar_game_state == GAME_PLAY) {
			//task_post_pure_msg(MAR_GAME_TASK_MAP_ID, MAR_GAME_MAP_MOVE_RIGHT_SIG);
			timer_set(AC_TASK_DISPLAY_ID,MAR_GAME_MAP_MOVE_RIGHT_SIG, 100, TIMER_PERIODIC);
		}
	}
		break;
	...
	case BUTTON_SW_STATE_LONG_PRESSED: {
		APP_DBG("[btn_down_callback] BUTTON_SW_STATE_LONG_PRESSED\n");
			if(mar_game_state == GAME_PLAY) {
				timer_set(AC_TASK_DISPLAY_ID,MAR_GAME_MAP_MOVE_RIGHT_TIMER_SIG, 100, TIMER_PERIODIC);
			}
		else{
	...
	case BUTTON_SW_STATE_RELEASED: {
		...
		if (ar_game_state != GAME_OFF) {
			...
			//task_post_pure_msg(MAR_GAME_TASK_MAP_ID, MAR_GAME_MAP_MOVE_RIGHT_SIG);
			timer_remove_attr(AC_TASK_DISPLAY_ID, MAR_GAME_MAP_MOVE_RIGHT_TIMER_SIG);
		}
		else {
